...
export const SideBarComponent = () => {
    ...
    return (
        <div className="relative p-1.5 antialiased">
            ...
            <div className={`flex gap-2 md:gap-4 px-2 md:px-4 py-1.5 items-center justify-between shadow-sm rounded-2xl border
                ${isDark
                ...
                {!isLoggedIn ? (
                    <Link
                        href="/login"
                        className={`flex justify-center items-center gap-2 px-2 py-2 rounded-xl text-center text-sm font-medium transition-colors duration-200
                            ${isDark
                ...
                <>
                    ...
                    <div className={`absolute right-2 top-24 z-20 w-80 rounded-2xl px-4 py-6 shadow-xl border backdrop-blur-md
                        ${isDark
                            ...
                            : 'bg-slate-600 text-gray-700 border-slate-300'}
                        animate-in fade-in slide-in-from-top-4 duration-200"
            `}>
                        {isLoggedIn && (
                            <>
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center justify-center w-18 h-18 font-bold text-gray-700 bg-gray-100 rounded-full dark:bg-gray-50">
                                        {user?.email.charAt(0).toUpperCase()}
                                    </div>

                                    <div className="text-center">
                                        <h3 className="text-md font-semibold text-slate-200 dark:text-slate-100">{user?.nom.charAt(0).toUpperCase() + user?.nom.slice(1)}</h3>
                                        <p className="text-sm font-semibold text-slate-200 dark:text-slate-300">{user?.email}</p>
                                    </div>
                                </div>

                                <div className="flex items-center justify-center pt-6 pb-3 border-b border-gray-500">
                                    <Link
                                        href="/client"
                                        className="px-6 py-2 bg-amber-100 text-gray-500 rounded-xl"
                                    >
                                        Gérer mon compte
                                    </Link>
                                </div>
                        ...
                        <div className="flex flex-col h-full justify-between gap-6 pt-2">
                                    ...
                                    <li key={item.id}>
                                        <a
                                            ...
                                            className="flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-colors
                                                ...
                                                dark:text-slate-300 dark:hover:text-amber-400 dark:hover:bg-amber-950/40"
                                        >
                                            {item.icon}
                                            {item.label}
                                        </a>
                            ...
                            <div className="pt-4 border-t border-slate-100 dark:border-slate-700 space-y-3">
                                {!isLoggedIn ? (
                                    <div className="flex flex-col items-center justify-center gap-4 w-full px-3 py-2 rounded-xl text-sm font-medium text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-950/30 transition-colors">
                                        <Link
                                            href="/login"
                                            className="flex justify-center items-center gap-2 px-4 py-2 rounded-xl text-center text-sm font-medium text-blue-200 hover:bg-blue-50 dark:text-blue-200 dark:hover:bg-blue-100 transition-colors duration-200"
                                        >
                                            <FiUser size={18} className="shrink-0" />
                                            <span className="shrink-0">Se connecter</span>
                                        </Link>
                                    ...
                                    <button
                                        ...
                                        className="flex items-center justify-center gap-4 w-full px-3 py-2 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/30 transition-colors">
